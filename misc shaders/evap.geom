...
float getNoise(vec3 vert){
	vec4 nv  = texture3D( Noise3, uNoiseFreq*vert );
...
ProduceVertex( float s, float t )
{
	...
	vec3 vel = uVelScale * vec3(-xVel*noise *noise, .8, 0);
	v = v + vel*tVel + 0.5*vec3(-uGravity/2., uGravity, 0.)*tVel*tVel;
